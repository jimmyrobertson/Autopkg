...
__all__ = ["AdobeFlashURLProviderWindows"]

UPDATE_XML_URL = ("http://fpdownload2.macromedia.com/"
                  "get/flashplayer/update/current/xml/version_en_win_pl.xml")

DOWNLOAD_TEMPLATE_URL = ("https://fpdownload.macromedia.com/"
                         "get/flashplayer/pdc/%s/install_flash_player_29_active_x.msi")

class AdobeFlashURLProviderWindows(Processor):
    ...
    def get_adobeflash_dmg_url(self):
        ...
        DOWNLOAD_URL = DOWNLOAD_TEMPLATE_URL % version
    ...
    def main(self):
        ...
        if "url" in self.env:
            ...
            return
        self.env["url"] , self.env["version"] = self.get_adobeflash_dmg_url()
        self.output("Found URL %s" % self.env["url"])
        self.output("Found Version %s" % self.env["version"])


if __name__ == '__main__':
    PROCESSOR = AdobeFlashURLProviderWindows()
